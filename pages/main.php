  <!-- ── INTRO ─────────────────────────────────────────────── -->
    <section id="intro" class="py-5">
        <div class="container-xxl p-3 rounded-5 bg-black bg-opacity-50">
            <div class="row justify-content-start align-items-start">
                <div class="col-md-5 text-center text-md-start">
                    <h1>
                        <div class="display-2">Vaszilij|EDC</div>
                        <div class="display-5 text-muted">Blogok - Írások és Minden ami Everyday Carry</div>
                    </h1>
                    <h6 class="my-4 text-muted">Ezeket a dolgokat cipelem...</h6>
                    <a href="#fresh_blogs" class="btn btn-danger btn-lg">Blog</a>
                </div>
                <div class="col-md-5 text-center d-none d-md-block my-3 ms-auto">
                    <img class="img-fluid bg-black bg-opacity-50 m-3 p-3 rounded-5" src="./assets/Logo_icon.png" alt="ebook cover">
                </div>
            </div>
        </div>
    </section>

    <!-- ── BLOGOK ─────────────────────────────────────────────── -->
    <section id="fresh_blogs" class="bg-black py-5">
        <div class="container-lg">
            <h1><span class="text-danger">•</span> Legfrissebb Blogok</h1>
            <div class="row g-4">

                <div class="col-md-4">
                    <div class="blog-card" style="background-image: url('./assets/blog_pictures/giant_mouse_ace_grand_main.jpeg');">
                        <div class="card-content">
                            <small class="text-danger text-uppercase fw-bold">Kések</small>
                            <h5>GiantMouse Ace Grand</h5>
                            <p class="text-muted">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                            <small class="text-muted">2025. feb. 18.</small>
                        </div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="blog-card" style="background-image: url('./assets/blog_pictures/kanetsune_kiridashi.jpeg');">
                        <div class="card-content">
                            <small class="text-danger text-uppercase fw-bold">Kések</small>
                            <h5>Kanetsune Kiridashi- a tökéletlenség bölcsessége</h5>
                            <p class="text-muted">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                            <small class="text-muted">2025. feb. 10.</small>
                        </div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="blog-card" style="background-image: url('./assets/blog_pictures/szamurajkardok_foldjen.jpeg');">
                        <div class="card-content">
                            <small class="text-danger text-uppercase fw-bold">Kések</small>
                            <h5>A szamurájkardok földjén...</h5>
                            <p class="text-muted">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                            <small class="text-muted">2025. jan. 30.</small>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </section>

<!-- ── CAROUSEL ─────────────────────────────────────────────── lehet galéria is--> 
<section id="carousel" class="bg-black bg-opacity-75 py-5">
    <div class="container-md">
        <div id="carouselExample" class="carousel slide" data-bs-ride="carousel">

            <!-- Indikátorok -->
            <div class="carousel-indicators">
                <button type="button" data-bs-target="#carouselExample" data-bs-slide-to="0" class="active"></button>
                <button type="button" data-bs-target="#carouselExample" data-bs-slide-to="1"></button>
                <button type="button" data-bs-target="#carouselExample" data-bs-slide-to="2"></button>
            </div>

            <div class="carousel-inner rounded">

                <!-- Dia 1 -->
                <div class="carousel-item active">
                    <div class="blog-card-carousel" style="background-image: url('./assets/blog_pictures/giant_mouse_ace_grand_main.jpeg');">
                        <div class="card-content">
                            <small class="text-danger text-uppercase fw-bold">Kések</small>
                            <h5>GiantMouse Ace Grand</h5>
                            <p class="text-muted">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                            <small class="text-muted">2025. feb. 18.</small>
                        </div>
                    </div>
                </div>

                <!-- Dia 2 -->
                <div class="carousel-item">
                    <div class="blog-card-carousel" style="background-image: url('./assets/blog_pictures/kanetsune_kiridashi.jpeg');">
                        <div class="card-content">
                            <small class="text-danger text-uppercase fw-bold">Kések</small>
                            <h5>Kanetsune Kiridashi – a tökéletlenség bölcsessége</h5>
                            <p class="text-muted">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                            <small class="text-muted">2025. feb. 10.</small>
                        </div>
                    </div>
                </div>

                <!-- Dia 3 -->
                <div class="carousel-item">
                    <div class="blog-card-carousel" style="background-image: url('./assets/blog_pictures/szamurajkardok_foldjen.jpeg');">
                        <div class="card-content">
                            <small class="text-danger text-uppercase fw-bold">Kések</small>
                            <h5>A szamurájkardok földjén...</h5>
                            <p class="text-muted">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                            <small class="text-muted">2025. jan. 30.</small>
                        </div>
                    </div>
                </div>

            </div>

            <!-- Nyilak -->
            <button class="carousel-control-prev" type="button" data-bs-target="#carouselExample" data-bs-slide="prev">
                <span class="carousel-control-prev-icon bg-danger rounded opacity-75"></span>
            </button>
            <button class="carousel-control-next" type="button" data-bs-target="#carouselExample" data-bs-slide="next">
                <span class="carousel-control-next-icon bg-danger rounded opacity-75""></span>
            </button>
        </div>
    </div>
</section>

<!--Oldal szerzőjének bemutatkozása-->
    <section id="about" class="bg-black py-5">
        <div class="container-lg">
            <div class="row align-items-center g-4">
            
            <!-- BALOLDAL: Szöveg -->
            <div class="col-lg-6">
                <h2 class="mb-3"><span class="text-danger">Naplónak</span> indult.</h2>
                <h4 class="text-muted mb-4">Bemutatató <span style="font-size: 1.8rem" class="text-danger">Bloggá</span> vált.</h4>
                
                <p>Aztán átalkult valami mássá. Ablakkák, amelyben kitekintek a világra, a világ meg betekinthet a gondolataimba: késekről, every day carry felszerelésekről, és az ezek mögött meghúzódó filozófiáról.</p>
                
                <p>Aztán ennél is több lett. Egy közösség, amelyben együtt, hasonló értékek mentén dolgozunk azért, hogy egy minősségi, kissé talán régimódi találkahely legyen ez az online térben.</p>
                
                <p>Balogh József vagyok, és azon dolgozom, hogy ez a közösség egyre nagyobba váljon, és együtt adhassunk tovább ezek az értékeket. Tarts velünk is!</p>
            </div>
            
            <!-- JOBBOLDAL: Kép -->
            <div class="col-lg-6">
                <img src="./assets/balogh.webp" class="img-fluid rounded-3 shadow-lg border-0 border-top border-danger border-3" alt="Bemutatkozás">
            </div>
            
            </div>
        </div>
    </section>  

    <!-- ── ÍRÁSOK ─────────────────────────────────────────────── -->
    <section id="writings" class="bg-black bg-opacity-75 py-5">
        <div class="container-lg">
            <h1><span class="text-danger">•</span> Írások</h1>
            <div class="row g-4">
                <div class="col-md-4">
                    <div class="card bg-dark bg-opacity-50 h-100 p-4 border-0 border-top border-danger border-3">
                        <small class="text-danger text-uppercase fw-bold ls-1 mb-2">Vers</small>
                        <h5 class="card-title">Bejegyzés 1</h5>
                        <p class="card-text text-secondary">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                        <small class="text-secondary mt-auto">2025. feb. 18.</small>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card bg-dark bg-opacity-50 h-100 p-4 border-0 border-top border-danger border-3">
                        <small class="text-danger text-uppercase fw-bold mb-2">Történet</small>
                        <h5 class="card-title">Bejegyzés 2</h5>
                        <p class="card-text text-secondary">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                        <small class="text-secondary mt-auto">2025. feb. 10.</small>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card bg-dark bg-opacity-50 h-100 p-4 border-0 border-top border-danger border-3">
                        <small class="text-danger text-uppercase fw-bold mb-2">Filozófia</small>
                        <h5 class="card-title">Bejegyzés 3</h5>
                        <p class="card-text text-secondary">Ez egy példa bejegyzés szövege, ami kicsit hosszabb...</p>
                        <small class="text-secondary mt-auto">2025. jan. 30.</small>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- ── BARÁTAINK ─────────────────────────────────────────── -->
    <section class="bg-black">
        <div class="container-lg my-5">
            <h2 class="mb-4">Barátaink</h2>
            
            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                
                <div class="col">
                    <div class="d-flex flex-column flex-lg-row align-items-center gap-3 py-3 border-top border-secondary border-opacity-25 h-100">
                        <div class="order-1 order-lg-0 flex-grow-1 text-center text-lg-start">
                            <span class="friends_names text-danger d-block mb-1">Bladeshop</span>
                            <p class="friends_desc text-muted mb-0">Késes webshop gyakori akciókkal és vevőbarát hozzáállással. Ha új kés kell, ne hagyd ki!</p>
                        </div>
                        <div class="order-0 order-lg-1 flex-shrink-0">
                            <img class="friends_logos img-thumbnail" src="./assets/other_edc_website_logos/bladeshop.jpg" alt="Bladeshop">
                        </div>
                    </div>
                </div>
                
                <div class="col">
                    <div class="d-flex flex-column flex-lg-row align-items-center gap-3 py-3 border-top border-secondary border-opacity-25 h-100">
                        <div class="order-1 order-lg-0 flex-grow-1 text-center text-lg-start">
                            <span class="friends_names text-danger d-block mb-1">Elemlámpa blog</span>
                            <p class="friends_desc text-muted mb-0">Minden, amit az elemlámpákról tudni szeretnél. Cikkek, bemutatók, illetve kuponok gyűjtőhelye.</p>
                        </div>
                        <div class="order-0 order-lg-1 flex-shrink-0">
                            <img class="friends_logos img-thumbnail" src="./assets/other_edc_website_logos/elemlampa.jpg" alt="Elemlámpa blog">
                        </div>
                    </div>
                </div>
                
                <div class="col">
                    <div class="d-flex flex-column flex-lg-row align-items-center gap-3 py-3 border-top border-secondary border-opacity-25 h-100">
                        <div class="order-1 order-lg-0 flex-grow-1 text-center text-lg-start">
                            <span class="friends_names text-danger d-block mb-1">Késvilág</span>
                            <p class="friends_desc text-muted mb-0">Hazai bolt és webáruház, rendkívül széles termékválasztékkal. Debrecenben személyesen is válogathatsz!</p>
                        </div>
                        <div class="order-0 order-lg-1 flex-shrink-0">
                            <img class="friends_logos img-thumbnail" src="./assets/other_edc_website_logos/kesvilag.jpg" alt="Késvilág">
                        </div>
                    </div>
                </div>
                
                <div class="col">
                    <div class="d-flex flex-column flex-lg-row align-items-center gap-3 py-3 border-top border-secondary border-opacity-25 h-100">
                        <div class="order-1 order-lg-0 flex-grow-1 text-center text-lg-start">
                            <span class="friends_names text-danger d-block mb-1">Magyar kések</span>
                            <p class="friends_desc text-muted mb-0">Webshop és közösség. Elsősorban a hazai készítők termékeivel foglalkozik, de nyitott egyéb irányokba is.</p>
                        </div>
                        <div class="order-0 order-lg-1 flex-shrink-0">
                            <img class="friends_logos img-thumbnail" src="./assets/other_edc_website_logos/mkszoveg.png" alt="Magyar kések">
                        </div>
                    </div>
                </div>
                
                <div class="col">
                    <div class="d-flex flex-column flex-lg-row align-items-center gap-3 py-3 border-top border-secondary border-opacity-25 h-100">
                        <div class="order-1 order-lg-0 flex-grow-1 text-center text-lg-start">
                            <span class="friends_names text-danger d-block mb-1">Késportál</span>
                            <p class="friends_desc text-muted mb-0">Magyarország legnagyobb késes tudásbázisa. Érdemes csatlakoznod a fórumhoz is!</p>
                        </div>
                        <div class="order-0 order-lg-1 flex-shrink-0">
                            <img class="friends_logos img-thumbnail" src="./assets/other_edc_website_logos/kesportal.jpg" alt="Késportál">
                        </div>
                    </div>
                </div>
                
                <div class="col">
                    <div class="d-flex flex-column flex-lg-row align-items-center gap-3 py-3 border-top border-secondary border-opacity-25 h-100">
                        <div class="order-1 order-lg-0 flex-grow-1 text-center text-lg-start">
                            <span class="friends_names text-danger d-block mb-1">ZBOSS</span>
                            <p class="friends_desc text-muted mb-0">Kések, edc felszerelések, túra és sok egyéb. Hazai webáruház, ahol a vevők elégedettsége a legfontosabb.</p>
                        </div>
                        <div class="order-0 order-lg-1 flex-shrink-0">
                            <img class="friends_logos img-thumbnail" src="./assets/other_edc_website_logos/zbboss.jpg" alt="ZBOSS">
                        </div>
                    </div>
                </div>
                
                <p class="text-white-50">Facebook közösségünk:
                <a href="https://www.facebook.com/groups/434204628313605" target="_blank" rel="noopener noreferrer" class="text-danger fs-4 opacity mx-1"><i class="bi bi-facebook"></i></a>
                </p>
            </div>
        </div>
    </section>